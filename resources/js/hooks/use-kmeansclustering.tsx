...
const useKMeansClustering = (transactions: string[][], k: number = 3, maxIterations: number = 100): ClusterResult => {
    ...
    useEffect(() => {
        ...
        const finalClusters = {
            pagi: sortedClusters[0].indices.map((idx) => transactions[idx]),
            siang: sortedClusters[1].indices.map((idx) => transactions[idx]),
            malam: sortedClusters[2].indices.map((idx) => transactions[idx]),
        };
