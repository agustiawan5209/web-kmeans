...
class KMeansClustering {
    ...
    public run(): ClusterResult {
        ...
        // Sort clusters by size to determine pagi, siang, malam
        ...
        const finalClusters = {
            pagi: sortedClusters[0].indices.map((idx) => this.transactions[idx]),
            siang: sortedClusters[1].indices.map((idx) => this.transactions[idx]),
            malam: sortedClusters[2].indices.map((idx) => this.transactions[idx]),
        };
        ...
        const featureImportance = this.uniqueItems.map((item) => {
            ...
            return {
                ...
                importance,
                clusterVariances: {
                    pagi: clusterStats[0].variance,
                    siang: clusterStats[1].variance,
                    malam: clusterStats[2].variance,
                },
            };
        });
